...
int main()
{
    auto ctx = p6::Context{{600, 600, "TRACKBALL CAMERA"}};
    ...
    TrackballCamera Camera{5.f, 0.0f, 0.0f};

    ctx.mouse_scrolled = [&](p6::MouseScroll scroll) {
        Camera.moveFront(scroll.dy);
    };
    ctx.mouse_dragged = [&](p6::MouseDrag drag) {
        Camera.rotateLeft(drag.delta.x * 250);
        Camera.rotateUp(drag.delta.y * 250);
    };
    ...
    ctx.update = [&]() {
        ...
        glm::mat4 MVMatrix     = glm::translate(Camera.getViewMatrix(), glm::vec3(0.0f, 0.0f, 0.0f));
        ...
        for (uint32_t i = 0; i < N_moon; i++)
        {
            MVMatrix = glm::translate(Camera.getViewMatrix(), glm::vec3(0.f, 0.f, 0.f));
