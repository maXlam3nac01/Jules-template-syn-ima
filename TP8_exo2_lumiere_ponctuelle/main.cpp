...
int main()
{
    auto ctx = p6::Context{{600, 600, "LUMIERE PONCTUELLE"}};
    ctx.maximize_window();
    ...
    const p6::Shader shader  = p6::load_shader("shaders/3D_light.vs.glsl", "shaders/pointLight.fs.glsl");
    const p6::Shader shader2 = p6::load_shader("shaders/3D_light.vs.glsl", "shaders/normals.fs.glsl");
    ...
    const GLint uLightPos_vs          = glGetUniformLocation(shader.id(), "uLightPos_vs");
    ...
    ctx.update = [&]() {
        ...
        glm::vec3 pointLightPos  = scalingMatrix * rotationMatrix * glm::vec4(pointLightPos, 1.0f);
        ...
        glUniform1f(uShininessFloat, 0.1f);
        glUniform3fv(uLightPos_vs, 1, glm::value_ptr(glm::vec3(Camera.getViewMatrix() * glm::vec4{1.f, 1.f, 1.f, 1.f})));
        ...
        MVMatrix     = glm::translate(MVMatrix, pointLightPos);
